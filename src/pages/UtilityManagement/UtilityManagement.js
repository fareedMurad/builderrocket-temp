...
const UtilityManagement = () => {
  ...
  const handleAddUtility = () => {
    dispatch(setSelectedUtility({})).then(() => {
      setShowUtilityModal(true);
    });
  };
  ...
  const handleFavorite = (utility) => {
    if (!isIsFavoriteLoading) {
      ...
      dispatch(updateIsFavorite(utility, !utility?.IsFavorite)).then(() => {
  ...
  const deleteUtilityModal = () => {
    return (
      <Modal show={showDeleteModal} onHide={cancelDeletion} centered size="lg">
        <Modal.Body>
